...
import stream.priceprocessor.OrderLineAggregationPriceProcessor;
import stream.priceprocessor.TaxPriceProcessor;

public class FuctionComposeAndThen {
	public static void main(String[] args) {
		...
		Order unprodessedOrder = new Order()
				.setId(1001L)
				.setOrderLines(Arrays.asList(
						new OrderLine().setAmount(BigDecimal.valueOf(1000)),
						new OrderLine().setAmount(BigDecimal.valueOf(2000))
				));
		List<Function<Order, Order>> priceProcessors = getPriceProcessors(unprodessedOrder);

		Function<Order, Order> mergedPriceProcessors = priceProcessors.stream()
				.reduce(Function.identity(), Function::andThen);

		Order processedOrder = mergedPriceProcessors.apply(unprodessedOrder);
